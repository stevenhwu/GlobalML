...
import gml.core.MHRatio;
import gml.core.Setting;
import gml.math.Constant;
import gml.math.Transformation;
import gml.prior.PriorBeta;
import gml.prior.PriorDist;
import gml.prior.PriorExp;
import gml.prior.PriorInvGamma;
import gml.prior.PriorNormal;
import gml.proposal.ProposalNormal;
...
public class GlobalPar {
	...
	private double tuneAlpha = 1;// 0.2;
	...
	public void setPriorSdScale(){
		
//		priorSpotScale = new PriorGamma(2, 1/2);
		priorSpotScale = new PriorNormal(1,0.25);
//		priorSpotScale = new PriorExp(2);
	}
	...
	public void updateMean(SpotPar[] sp, Likelihood li, double tune) {
		...
		// double[] newSd = ProposalScale.nextValue(meanSd, 1/tune);
		// double[] newMu = ProposalScale.nextValue(meanMu, tune);
		// double[] newSd = ProposalScale.nextTruncatedValue(meanSd, tune,
		// Constant.MIN_SD, Setting.getSdCap());

		// System.out.println("MU\t"+mu+"\t"+ArrayUtils.toString(newMu));
		// System.out.println("SD\t"+sd+"\t"+ArrayUtils.toString(newSd));
...
//		ProposalScale.nextTruncatedValue(spotScale, tune, 0.001, 2);
	...
	public void updateDelta(SpotPar[] sp, Likelihood li, double tune) {
		...
		// double[] newPhi = ProposalNormal.nextTruncatedValue(phi, tune
		// * tuneSdDelta, Constant.PHI_MIN, Constant.PHI_MAX);
		// double[] newPhi = ProposalUniform.nextValue(Constant.PHI_MIN,
		// Constant.PHI_MAX);
		// double[] newLambda = ProposalScale.nextValue(lambda, tune);
		// double[] newPhi = ProposalScale.nextTruncatedValue(phi, 0.9,
		// Constant.PHI_MIN, Constant.PHI_MAX);
	...
	public void updateProb1(SpotPar[] sp, Likelihood li, double tune) {
		// System.out.println(newAlpha[0]);
		// double[] newAlpha = ProposalNormal.nextTruncatedValue(alphaProb1,
		// 0.3, 0 , 5);
		...
		// double[] newAlphaPi = ProposalScale.nextValue(alphaPi, 0.95);
		...
		// double[] newPiMu = ProposalScale.nextValue(piMu, tune);
		// double[] newPiSd = ProposalScale.nextTruncatedValue(piSd, tune,
		// Constant.MIN_SD, Setting.getProbSdCap());

		// System.out.println("alpha1:\t"+alpha1+"\tbeta1:\t"+beta1);
		// System.out.println(ArrayUtils.toString(newAlpha1));
		// System.out.println(ArrayUtils.toString(newBeta1));
		...
		// + priorProbSd.getLogPrior(newPiSd[0]);
		// double newLikeli = li.condLikeliPi(sp, newPiMu[0], newPiSd[0]);
		...
		double newPosterior = newLikeli + newPrior;
		// System.out.println(li.getPriorProb(GPI)
		// +"\t"+priorProbMu.getLogPrior(piMu) +"\t"
		// + priorProbSd.getLogPrior(piSd) +"\n\tZZ:"+newLikeli
		// +"\t"+li.getParamLikelihood(GPI));
		boolean accept = MHRatio.acceptTemp(xGivenNewX, newXGivenX,
				oldPosterior, newPosterior, temperature);

		// System.out.println("oldPrior: "+li.getPriorProb(GPI)+" newPrior: "+newPrior+
		// "A: "+accept);
		// accept = MHRatio.accept(0, 0, oldPosterior, newPosterior);
		// System.out.println(xGivenNewX);
		// System.out.println(newXGivenX);
		// System.out.println(oldPosterior);
		// System.out.println(newPosterior);
		if (accept) {
			// System.out.println(piMu+"\t"+newPiMu[0]+"\t"+
			// piSd+"\t"+newPiSd[0]+"\tLikeli:"+li.getParamLikelihood(GPI)
			// +"\t"+newLikeli+"\tPrior:"+ li.getPriorProb(GPI) +"\t"+newPrior);
			piMu = newPiMu[0];
			piSd = newPiSd[0];
	...
	public void updateAlphaPi(SpotPar[] sp, Likelihood li, double tune) {
		...
		double[] newAlphaPi = ProposalNormal.nextTruncatedValue(alphaPi, tune,
				0.01, 10);
		// , 0, 10);
		// double[] newAlphaPi = ProposalNormal.nextTruncatedValue(alphaPi,
		// tune, 0, 10);

		// double[] newPiSd = ProposalNormal.nextTruncatedValue(piSd, tune,
		// Constant.MIN_SD, Setting.getProbSdCap());
	...
	public void updateProb2(SpotPar[] sp, Likelihood li, double tune) {
		...
		// double[] newRhoMu = ProposalScale.nextValue(rhoMu, tune);
		// double[] newRhoSd = ProposalScale.nextTruncatedValue(rhoSd, tune,
		// Constant.MIN_SD, Setting.getProbSdCap());
		// newRhoSd[0] = 10;
	...
	public void updateAlphaRho(SpotPar[] sp, Likelihood li, double tune) {
		...
		// double[] newRhoSd = ProposalNormal.nextTruncatedValue(rhoSd, tune,
		// Constant.MIN_SD, Setting.getProbSdCap());
		...
		double[] newAlphaRho = ProposalNormal.nextTruncatedValue(alphaRho,
				tune, 0.01, 10);
		// double[] newAlphaRho = ProposalScale.nextTruncatedValue(alphaRho,
		// 0.95, 0.1, 5);
	...
	@Deprecated
	public void updatePiSd(SpotPar[] sp, Likelihood li, double tune) {
		// double[] newAlpha = ProposalNormal.nextTruncatedValue(alphaProb1,
		// 0.3, 0 , 5);
	
		double[] newPiSd = ProposalNormal.nextTruncatedValue(Math.pow(piSd, 2),
				tune, Constant.MIN_SD, Setting.getProbSdCap());
		newPiSd[0] = Math.sqrt(newPiSd[0]);
	
		double[] newPiMu = new double[] { piMu, 0, 0 };
		// newPiSd = new double[] {piSd, 0, 0};
		double[] newAlphaPi = new double[] { alphaPi, 0, 0 };
		// double[] newAlpha = ProposalScale.nextTruncatedValue(alphaProb1, 0.5,
		// 0, 5);
		// double[] newAlpha =ProposalNormal.nextTruncatedValue(alphaProb1, 0.1,
		// 0 , 5);
		double realMu = newPiMu[0] * newAlphaPi[0];
		double realSd = newPiSd[0] * newAlphaPi[0];
	...
	@Deprecated
	public void updateRhoSd(SpotPar[] sp, Likelihood li, double tune) {
	
		double[] newRhoSd = ProposalNormal.nextTruncatedValue(Math
				.pow(rhoSd, 2), tune, Constant.MIN_SD, Setting.getProbSdCap());
		newRhoSd[0] = Math.sqrt(newRhoSd[0]);
		// double[] newAlphaRho = ProposalNormal.nextTruncatedValue(alphaRho,
		// tune,
		// 0, 5);
