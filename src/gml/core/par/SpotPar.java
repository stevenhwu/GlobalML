...
public class SpotPar {
	...
	// static NormalDistribution nd2 = new NormalDistribution();
	// static ExpUniExpDistribution eueDist = new ExpUniExpDistribution();

	// static Likelihood tempLi;
	...
	public void updateMuD(GlobalPar gp, Likelihood li, double tune) {
		...
		double newLikeli = li.calLogLikeli(thisSpot, tempParMuDLikeli(newMu,
				newD));
	...
	public void updatePiRho(GlobalPar gp, Likelihood li, double tune) {
		...
		// Normal without boundary,
		// P(newX | X) = P(X | new X)
		// double newXGivenX = NormalDistribution.logPdf(newPi, pi, tune)
		// + NormalDistribution.logPdf(newRho, rho, tune);
		//
		// double xGivenNewX = NormalDistribution.logPdf(pi, newPi, tune)
		// + NormalDistribution.logPdf(rho, newRho, tune);
		...
		double newPost = newLikeli + newPrior;
		// boolean accept = MHRatio.accept(xGivenNewX, newXGivenX, spotPosterLi,
		// newPost);
		...
		// if (spotIndex == 1) {
		// if(gp.getPiSd() <0.01){
		// System.out.println(pi + " " + newPi + " " + rho + " " + newRho
		// + "\n" + gp.getPiSd() + " " + gp.getRhoSd() + "\t"
		// + newPrior);
		// }}
		// TODO cleanup
	...
	public double getLikelihood() {
		return spotPosterLi; // TODO remove later, just for debugging
	}
