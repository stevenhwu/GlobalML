...
public class HeapSort {
	 ...
	 * 
	 * for (int i = 0; i < test.length; i++) { test[i] = new
	 * ComparableDouble(MathUtils.nextInt(testSize * 10)); }
	 * 
	 * sort(test); for (int i = 0; i < test.length; i++) {
	 * System.out.print(test[i] + " "); } System.out.println();
	 * 
	 * // test index to Vector of Comparable objects
	 * 
	 * Vector testv = new Vector(); int[] indices = new int[testSize];
	 * 
	 * for (int i = 0; i < testSize; i++) { testv.addElement(new
	 * ComparableDouble(MathUtils.nextInt(testSize * 10))); }
	 * 
	 * sort(testv, indices); for (int i = 0; i < test.length; i++) {
	 ...
	 * 
	 * double[] testd = new double[testSize]; //int[] indices = new
	 * int[testSize];
	 * 
	 * for (int i = 0; i < testSize; i++) { testd[i] =
	 * MathUtils.nextInt(testSize * 10); }
	 * 
	 * sort(testd, indices); for (int i = 0; i < test.length; i++) {
	 * System.out.print(testd[indices[i]] + " "); } System.out.println();
	 * 
	 * }
	 */
	 ...
	 */
	private static void adjust(AbstractList array, int[] indices, int lower,
			int upper) {
		...
		while (k <= upper) {
			if ((k < upper)
					&& (((Comparable) array.get(indices[k - 1]))
							.compareTo(array.get(indices[k])) < 0)) {
				k += 1;
			}
			if (((Comparable) array.get(indices[j - 1])).compareTo(array
					.get(indices[k - 1])) < 0) {
				temp = indices[j - 1];
