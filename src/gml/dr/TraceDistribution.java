...
public class TraceDistribution {
	...
	private void analyseDistribution(double[] values, int stepSize) {
		...
		calculateHPDInterval(0.975, values, indices);
		ESS = values.length;
	...
	/**
	 * @param proportion
	 *            the proportion of probability mass oncluded within interval.
	 */
	private void calculateHPDInterval(double proportion, double[] array,
			int[] indices) {

		double minRange = Double.MAX_VALUE;
		int hpdIndex = 0;

		int diff = (int) Math.round(proportion * array.length);
